...
```mermaid
flowchart TD
    subgraph "1️⃣ Сбор данных"
        A[("🖥️ Экран пользователя")]
        B[("🏷️ Активное окно")]
        C[("👤 User ID из .env")]
    end
    
    subgraph "2️⃣ Screener Service"
        D["📸 Захват экрана
        (screencapture/import/PowerShell)"]
        E["🖼️ Конвертация в PNG"]
        F["📦 Кодирование в base64"]
        G["📨 Формирование JSON"]
    end
    
    subgraph "3️⃣ Очереди RabbitMQ"
    ...
    subgraph "5️⃣ Notifier Service"
        N["📥 Получение результатов"]
        O["🖥️ Системное уведомление"]
        P["📝 Логирование"]
    end
    
    subgraph "6️⃣ Внешние сервисы"
        Q[("🌐 OpenRouter API
        GPT-4o модель")]
    end
...
Если хотите контрибьютить в проект, пишите в Telegram: @ваш_ник
